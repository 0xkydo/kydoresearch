...
// src/oncall/prompt.ts
var ONCALL_SYSTEM_PROMPT = `You are the operational on-call engineer for a
...
function assessmentPrompt(stream, forcedReason) {
...
function repairPrompt(reportPath, evidencePath, runtimeRoot, assessment) {
...
// src/oncall/supervisor.ts
var DEFAULT_MODEL = "openai-codex/gpt-5.6-sol";
var DEFAULT_THINKING = "high";
var MAX_STREAM_WINDOW_BYTES = 96 * 1024;
var MAX_CAPTURED_STDERR_BYTES = 64 * 1024;
var MAX_AGENT_OUTPUT_BYTES = 2 * 1024 * 1024;
var ANALYST_TIMEOUT_MS = 10 * 6e4;
var REPAIR_TIMEOUT_MS = 60 * 6e4;
var STABLE_RUN_MS = 30 * 6e4;
var ACTIVE_PHASE_PREFIXES = ["init.", "loop."];
var TERMINAL_PHASES = /* @__PURE__ */ new Set(["done", "ready"]);
function parseSupervisorArgs(argv, cwd = process.cwd(), runtimeRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../..")) {
  const forwarded = [];
  ...
  let scanIntervalMs = 3e4;
  let stalledAfterMs = 90 * 6e4;
  let restartBaseDelayMs = 5e3;
  let restartMaxDelayMs = 5 * 6e4;
  let maxRestarts = 10;
...
function deterministicIncident(state, nowMs, lastProgressAt, stalledAfterMs) {
  const phase = typeof state?.phase === "string" ? state.phase : void 0;
  const recovery = isRecord(state?.recovery) ? state.recovery : void 0;
  ...
  const active = phase !== void 0 && (ACTIVE_PHASE_PREFIXES.some((prefix) => phase.startsWith(prefix)) || phase === "church" || phase === "god");
...
function processExitIncident(exit, state) {
  const phase = typeof state?.phase === "string" ? state.phase : void 0;
  if (exit.code === 0 && (phase === void 0 || TERMINAL_PHASES.has(phase) || phase === "paused")) {
    return null;
  }
  return {
    category: "process-crash",
    problem: `The supervised Pi process exited unexpectedly (code ${exit.code ?? "null"}, signal ${exit.signal ?? "none"}).`,
    why: phase ? `Durable AutoResearch phase "${phase}" was not terminal when Pi exited.` : "Pi exited nonzero before a terminal AutoResearch checkpoint was available.",
    evidence: [JSON.stringify({ exit, phase: phase ?? null })]
  };
}
function parseOncallAssessment(text) {
  ...
  const confidence = /* @__PURE__ */ new Set(["low", "medium", "high"]);
  if (typeof value.catastrophic !== "boolean" || typeof value.category !== "string" || !categories.has(value.category) || typeof value.confidence !== "string" || !confidence.has(value.confidence) || typeof value.problem !== "string" || typeof value.why !== "string" || !stringArray(value.errorLogs) || !stringArray(value.possibleRootCauses) || typeof value.repairable !== "boolean" || !stringArray(value.repairScope) || typeof value.restartRecommended !== "boolean") {
    return null;
  }
  return value;
}
function shouldIntervene(assessment, deterministic, matchingAssessments = 1) {
  if (deterministic) return true;
  return assessment?.catastrophic === true && assessment.confidence === "high" && matchingAssessments >= 2;
}
var OncallSupervisor = class {
  options;
  stopping = false;
  current = null;
  ...
  streamWindow = "";
  pendingStreamEvidence = false;
  restartCount = 0;
  seenIncidentSignatures = /* @__PURE__ */ new Set();
  lastSemanticIncident = "";
  matchingSemanticAssessments = 0;
  constructor(options) {
...
`);
  ...
  async monitorCurrentProcess(current) {
    while (!this.stopping) {
      const result = await Promise.race([
        current.exit.then((exit) => ({ type: "exit", exit })),
        delay(this.options.scanIntervalMs).then(() => ({ type: "scan" }))
      ]);
      if (this.stopping) return null;
      const batch = this.collectStreamBatch();
      const deterministic = result.type === "exit" ? processExitIncident(result.exit, batch.state) : deterministicIncident(
        batch.state,
        Date.now(),
      ...
      const assessment = await this.runOncallAnalyst(
        batch.text,
        deterministic ? `${deterministic.problem}
${deterministic.why}
${deterministic.evidence.join("\n")}` : void 0
      );
      const matchingAssessments = deterministic ? 0 : this.recordSemanticAssessment(assessment);
  ...
  recordSemanticAssessment(assessment) {
  ...
  launchPi(autoResume) {
    ...
    const exit = new Promise((resolve3) => {
      child.once("error", (error) => {
        this.recordStream("pi.error", error.message);
        resolve3({ code: 1, signal: null });
      });
    ...
    return {
      ...
      terminate: async () => {
        ...
        await Promise.race([exit, delay(5e3)]);
...
${chunk}`);
  ...
  async runOncallAnalyst(stream, forcedReason) {
    ...
    if (result.code !== 0) {
      this.recordStream(
        "oncall.error",
        `analyst exited ${result.code}: ${clipTail(result.stderr, 4096)}`
      );
      return null;
    }
  ...
  async handleIncident(assessment, evidence) {
...
`);
  ...
  async runCodexRepair(files, assessment) {
    ...
    const finalText = fs.existsSync(lastMessage) ? fs.readFileSync(lastMessage, "utf8") : assistantTextFromCodexJsonl(result.stdout);
    const parsed = parseRepairResult(finalText);
    if (parsed) return parsed;
    return {
      status: result.code === 0 ? "unknown" : "failed",
      summary: result.code === 0 ? "Codex completed without a valid structured repair result." : `Codex exited with code ${result.code}: ${clipTail(result.stderr, 2048)}`,
      filesChanged: [],
      validation: [],
      remainingRisk: "The repair outcome could not be verified from the agent contract."
    };
  }
  createIncidentFiles(signature) {
...
function fallbackAssessment(incident) {
  return {
    catastrophic: true,
...
function parseRepairResult(text) {
  const value = parseJsonObject(text);
  if (!value) return null;
  const statuses = /* @__PURE__ */ new Set(["fixed", "no-code-fix", "blocked"]);
  if (typeof value.status !== "string" || !statuses.has(value.status) || typeof value.summary !== "string" || !stringArray(value.filesChanged) || !stringArray(value.validation) || typeof value.remainingRisk !== "string") {
    return null;
  }
  return value;
}
...
function assistantTextFromPiJsonl(stdout) {
  const messages = [];
  for (const line of stdout.split(/\r?\n/)) {
    ...
    try {
      ...
      if (Array.isArray(content)) {
        messages.push(
          content.filter((part) => isRecord(part)).map((part) => part.type === "text" && typeof part.text === "string" ? part.text : "").join("")
        );
      }
    } catch {
    }
...
function assistantTextFromCodexJsonl(stdout) {
  const messages = [];
  for (const line of stdout.split(/\r?\n/)) {
    try {
      const event = JSON.parse(line);
...
async function captureProcess(command, args, cwd, tracePath, timeoutMs) {
...
`;
      ...
      setTimeout(() => terminateChild(child, true), 5e3).unref();
    }, timeoutMs);
    timeout.unref();
  });
}
...
function progressFingerprint(state) {
  const ideas = Array.isArray(state.ideas) ? state.ideas.map(
    (idea) => isRecord(idea) ? {
      id: idea.id,
      status: idea.status,
...
function walkLogFiles(directory, output) {
  let entries;
  try {
  ...
  for (const entry of entries) {
    ...
    } else if (entry.isFile() && (entry.name.endsWith(".log") || entry.name.endsWith(".ndjson"))) {
      output.push(target);
    }
  }
}
function incidentMarkdown(assessment) {
  const bullets = (values) => values.length > 0 ? values.map((value) => `- ${value}`).join("\n") : "- None captured.";
...
function incidentSignature(assessment, evidence) {
  return createHash("sha256").update(assessment.category).update("\0").update(assessment.problem).update("\0").update(clipTail(evidence, 16384)).digest("hex");
}
...
function clipTail(value, maximumBytes) {
...
async function main(argv = process.argv.slice(2)) {
  const entryDirectory = path2.dirname(fileURLToPath2(import.meta.url));
  const runtimeRoot = path2.resolve(
    entryDirectory,
    path2.basename(entryDirectory) === "bin" ? ".." : "../.."
  );
  let parsed;
  try {
